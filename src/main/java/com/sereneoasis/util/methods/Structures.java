...
public class Structures {
    ...
    public static void spawnStructure(Location loc, String name) {
        SereneAbilities.getPlugin().getServer().getStructureManager().loadStructure(NamespacedKey.minecraft(name)).place(loc, false, StructureRotation.NONE, Mirror.NONE, 0, 1, new Random());
    }

    public static void spawnStructureFill(Location loc, String name) {
        Structure structure = SereneAbilities.getPlugin().getServer().getStructureManager().loadStructure(NamespacedKey.minecraft(name));
